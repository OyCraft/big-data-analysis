# -*- coding: utf-8 -*-
"""
Created on Tue Oct 26 10:11:38 2021
...
def forFM(xx, yy, txx, k, maxI, sig = 0.1, eta = 0.001):
    ...
    for ite in range(maxI * n + 1):
        ...
        for f in range(k):
            ydif = ydif + 0.5 * (np.dot(xi, V[:, f])**2 - np.dot(xi**2, V[:, f]**2))           
        w0 = w0 - 2 * eta * ydif
        for l in range(p):
            w[l] = w[l] - 2 * eta * xi[l] * ydif
            for f in range(k):
                V[l, f] = V[l, f] - 2 * eta * xi[l] * ydif * (np.dot(xi, V[:, f]) - V[l, f] * xi[l])
...
#下面这个函数用来遍历隐向量维数和最大全样本迭代次数，输出各自的AUC值
def multiAUC(train_x, train_y, test_x, test_y, n, m, grid):         
    AUC = np.zeros([n, m])                        #n是迭代次数，m是隐向量维数
    ...
    for i in range(1, n + 1):
        for j in range(1, m + 1):
            np.random.seed(seed)
            y1, y2, w0, w, V = FM(train_x, train_y, test_x, k = j, maxI = i, reg = False)
            TPR = np.zeros(len(grid) + 1)
            FPR = np.zeros(len(grid) + 1)
            pos = 1
            for kk in np.flip(grid):
                pred_y = copy.deepcopy(y2[:, 0])
                pred_y[pred_y>=kk] = 1
                pred_y[pred_y<kk] = 0
                TPR[pos] = sum((pred_y==1)&(test_y==1)) / sum(test_y==1)
                FPR[pos] = sum((pred_y==1)&(test_y==0)) / sum(test_y==0)
                AUC[i - 1, j - 1] = AUC[i - 1, j - 1] + (TPR[pos] + TPR[pos - 1]) * (FPR[pos] - FPR[pos - 1]) / 2
                pos = pos + 1
            '''
            TPR = 0
            FPR = 0
            for k in np.flip(grid):
                ...
                pred_y[pred_y>=k] = 1
                pred_y[pred_y<k] = 0
                newTPR = sum((pred_y==1)&(test_y==1)) / sum(test_y==1)
                newFPR = sum((pred_y==1)&(test_y==0)) / sum(test_y==0)
                #计算梯形面积
                AUC[i - 1, j - 1] = AUC[i - 1, j - 1] + (newTPR + TPR) * (newFPR - FPR) / 2
                TPR = newTPR
                FPR = newFPR
            '''
    return AUC
...
#查看运行结果
grid = np.arange(start = 0, stop = 1, step = 0.001)
tpr, fpr = reportROC(y2, test_y, grid, 0.5)
...
#寻找最优参数
#grid = np.arange(start = 0, stop = 1, step = 0.001)
#auc = multiAUC(train_x, train_y, test_x, test_y, 3, 7, grid)
