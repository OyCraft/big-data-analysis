...
import torchvision
#安装：conda install torchvision -c pytorch
...
#上述对data_transforms进行了两种变换
#transforms.ToTensor() 将PILImage转变为torch.FloatTensor的数据形式
#Normalize则是对tensor进行的

trainset = torchvision.datasets.CIFAR10(root='./data', train=True,
                                        download=True, transform=transform)
#CIFAR10是torchvision提供的图片数据集，含有各类32*32大小点阵图
trainloader = torch.utils.data.DataLoader(trainset, batch_size=4,
                                          shuffle=True, num_workers=0)
#原程序中num_workers=2会报错：BrokenPipeError: [Errno 32] Broken pipe
#根据网络意见应改为num_workers=0，下同
